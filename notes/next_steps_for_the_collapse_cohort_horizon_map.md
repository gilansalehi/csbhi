...
> **Programme update — September 2026.** The mature scale mode is fixed conditionally by \(\Gamma=a^2\), but the throat clock and null affine transfer remain open. For an independently established local Weyl factor, child marginality requires \(\theta_P+2k_P(\ln\Omega_{\rm W})=0\) and \(\mathcal T=C_\lambda\Omega_{{\rm W},H}^2\). The programme must derive how those local quantities project onto \(\Gamma\). See [Mature Gamma Dynamics and the Conditional Radiation Handoff](note.html?src=mature_gamma_dynamics_and_conditional_radiation_handoff).
...
7. **Launch the mature dual scale.** The mature branch now has \(\Gamma=a^2=r^{-2}\) and a closed Friedmann first integral once \(H_\Lambda,\eta_m,\eta_r\) are supplied. Derive from the throat the required handoff two-jet, test the conditional stationary-parent \(\Rightarrow w=1/3\) theorem, and independently relate the timelike scale mode to the null affine transfer.
