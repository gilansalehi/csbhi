...
> **Programme update — September 2026.** The mature scale mode is fixed conditionally by \(\Gamma_{\rm GD}=a^2\), but the throat clock and null affine transfer remain open. For an independently established local Weyl factor, child marginality requires \(\theta_P+2k_P(\ln\Omega_{\rm W})=0\) and \(\mathcal T=C_\lambda\Omega_{{\rm W},H}^2\). The programme must derive how those local quantities project onto \(\Gamma_{\rm GD}\). See [Mature Gamma Dynamics and the Conditional Radiation Handoff](note.html?src=mature_gamma_dynamics_and_conditional_radiation_handoff).
...
**Status:** Research plan. The causal form and homogeneous shell map are established conditionally; the clock law and its extension to a completed spacetime remain open. Tiered **[E]** established, **[C]** candidate CSBHI correspondence, **[O]** open.
...
3. **Write the two null embeddings — completed; their old unit-lapse closure is superseded.** See [The Collapse-to-Child Boundary System](note.html?src=collapse_to_child_boundary_system). The separate embeddings remain exact in their own metrics. Their cross-frame clock and affine normalization still require a throat map.
...
5. **Compute the null optical transfer — parent reduction completed; throat projection open.** See [Optical Transfer Through a Matter-Filled Parent Collapse](note.html?src=optical_transfer_through_compensated_ltb_collapse), [The Conformal Dictionary and the Pre-FLRW Causal Interval](note.html?src=conformal_dictionary_and_pre_flrw_causal_interval), and [Mature Gamma Dynamics and the Conditional Radiation Handoff](note.html?src=mature_gamma_dynamics_and_conditional_radiation_handoff). The within-parent integrating factor obeys \(d\ln\beta/dt_P=-\dot Y'/Y'\). A genuine local Weyl factor would give \(\mathcal T=C_\lambda\Omega_{{\rm W},H}^2\), but the throat has not proved \(\Omega_{\rm W}=\Gamma_{\rm GD}\).
6. **Check the asymptotes — old endpoint series retained as diagnostics.** See [Asymptotic Consistency of the Collapse-Cohort Map](note.html?src=asymptotic_consistency_of_the_collapse_cohort_map). The old model's formation and late series remain useful, but its unit-lapse, raw mass-tail, and child-reconstruction coefficients must be recalculated through the completed throat map.
7. **Launch the mature dual scale.** The mature branch now has \(\Gamma_{\rm GD}=a^2=r^{-2}\) and a closed Friedmann first integral once \(H_\Lambda,\eta_m,\eta_r\) are supplied. Derive from the throat the required handoff two-jet, test the conditional stationary-parent \(\Rightarrow w=1/3\) theorem, and independently relate the timelike scale mode to the null affine transfer.
