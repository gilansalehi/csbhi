...
> **Superseding scale audit — September 2026.** The horizon-name guardrails and angular scale relation below remain current. The boundary density identity in Section 6 is only an isometric/parent-time benchmark. Canonical notation now distinguishes the mature dual scale \(\Gamma=a^2\) from a local Weyl factor \(\Omega_{\rm W}\); this note's \(\Omega_\Sigma\) is boundary scale data whose relation to either object must be derived. See [Mature Gamma Dynamics and the Conditional Radiation Handoff](note.html?src=mature_gamma_dynamics_and_conditional_radiation_handoff).
