...
[*Mature Gamma Dynamics and the Conditional Radiation Handoff*](note.html?src=mature_gamma_dynamics_and_conditional_radiation_handoff) closes the scale mode after a successful reciprocal handoff. The ruler ratio is \(\Gamma=a/r=a^2=r^{-2}\), with \(\dot\Gamma/\Gamma=2H\); the flat matter–radiation–\(\Lambda\) background becomes one first integral for \(\Gamma\), and radiation appears as the trace-free constant kinetic term. The same note translates the handoff two-jet into initial data for \(\Gamma\) and proves a conditional stationary-parent \(\Rightarrow w=1/3\) statement. The throat still has to establish its ruler and time-map premises and relate \(\Gamma\) to \(\Omega_{\rm W}\) and the null affine transfer.
...
Two analytic threads now meet at the same theory boundary. The nonspherical thread must derive the birth-limit local Weyl behavior and apply that actual GD completion to the stable two-sheet \((A_1,A_1)\) crease. The spherical thread must derive how \(\Omega_{\rm W}\), \(\Gamma\), the cohort lapse, and the null affine transfer arise as projections of one throat geometry; solve child marginality; and test whether the local and future-jet determinations of \(\mathcal J\) agree—without setting \(R_b^P=R_A^C\). A realistic equation of state and numerical event-horizon sweep remain later amplitude tests.
...
The revised ledger explicitly rejects the former isometric shortcut \(L=2GM_P/c^2\). A stellar parent can produce a cosmological child scale only through the still-open throat transfer. The mature ruler scale \(\Gamma=a^2\) is no longer functionally free after handoff, but that does not fix its launch, the cohort lapse, or the null affine transfer. The remaining formation data are the relation among \(\Gamma\), \(\Omega_{\rm W}\), the older \(K(q)\), and the complete child metric. The galaxy sector separately retains two testable horizon choices: a fixed \(cH_\Lambda\) scale or an evolving \(cH(z)\) scale.
